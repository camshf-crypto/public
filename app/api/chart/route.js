...
function generateChartHTML(type, labels, data, title, color) {
  ...
  if (type === 'bar') {
    const bars = labels.map((label, i) => {
      ...
      return `<g transform="translate(${i * barWidth}%, 0)">
        <rect x="10%" y="${100 - height}%" width="80%" height="${height}%" fill="${color}" rx="3"/>
        <text x="50%" y="105%" text-anchor="middle" font-size="12" fill="#666">${label}</text>
        <text x="50%" y="${95 - height}%" text-anchor="middle" font-size="11" fill="#333">${data[i]}</text>
      </g>`
    }).join('')
    return `<div style="padding:16px">
      <div style="font-size:13px;font-weight:700;color:#333;margin-bottom:12px;text-align:center">${title}</div>
      <svg viewBox="0 0 400 200" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:160px">${bars}</svg>
    </div>`
  }
  ...
  if (type === 'line') {
    ...
    return `<div style="padding:16px">
      ...
      <svg viewBox="0 0 400 210" xmlns="http://www.w3.org/2000/svg" style="width:100%;height:160px">
        <polyline points="${points}" fill="none" stroke="${color}" stroke-width="2.5"/>${dots}
      </svg>
    </div>`
  }

  return `<div style="padding:16px;text-align:center;color:#aaa">그래프 생성 중...</div>`
}